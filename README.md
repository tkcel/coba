...
| コマンド | 説明 |
|----------|------|
| `/jira-check` | チケット確認（引数: `my-tickets` / `overdue` / `today` / `daily` / `weekly`） |
| `/jira-create-ticket` | チケット作成（対話型、確認付き） |
| `/jira-update-ticket` | ステータス変更・コメント追加（確認付き） |
| `/jira-issue` | チケット詳細表示 |
...
| `jira.md` | `scripts/jira.sh` | Jira 連携ルール（JQL、フォーマット、エラー処理） |
...
├── jira-check.md         # Jira チケット確認
├── jira-create-ticket.md # Jira チケット作成
├── jira-update-ticket.md # Jira ステータス変更・コメント
└── jira-issue.md         # Jira チケット詳細表示
```
...
COBA は外部サービスと連携して、日常業務の情報を集約する。

| サービス | 用途 | 接続方式 | 必須 |
|----------|------|----------|:----:|
| Google Calendar | `/generate-daily` で予定を取得 | MCP | |
| Slack | チャンネル検索、メッセージ送信、スタンドアップ生成など | プラグイン | |
| Jira | チケット管理、ステータス確認、起票 | REST API (curl) | |

### Slack

`.claude/settings.json` で Slack プラグインとして有効化。
...
Jira Cloud REST API を `scripts/jira.sh`（curl ベースのシェルスクリプト）で連携。Atlassian MCP の代わりに、軽量で安定した自前実装を採用。

#### セットアップ

##### 1. API トークンの発行

[Atlassian API トークン管理](https://id.atlassian.com/manage-profile/security/api-tokens) でトークンを発行する。
...
| 項目 | 説明 | 例 |
|------|------|-----|
| `JIRA_DOMAIN` | Atlassian ドメイン | `irodas.atlassian.net` |
| `JIRA_EMAIL` | ログインメール | `t.nemoto@irodas.com` |
| `JIRA_API_TOKEN` | 発行した API トークン | `ATATT3x...` |

> **セキュリティ**: 認証情報はリポジトリ外（`~/.config/jira/`）に保管。`.gitignore` にもセーフガードあり。
...
使用するプロジェクトキーを確認し、`knowledge/me.md` の Jira セクションに記入する。

> **Tips**: `/onboarding` を実行すれば、上記の手順を対話形式でガイドしてくれる。
...
`scripts/jira.sh` は以下のサブコマンドを提供する。秘書経由で自然言語でも操作可能。

| サブコマンド | 説明 | 例 |
|---|---|---|
| `myself` | 自分の情報取得 | `./scripts/jira.sh myself` |
| `search "<JQL>"` | JQL でチケット検索 | `./scripts/jira.sh search "assignee = currentUser() AND resolution = Unresolved"` |
| `issue <KEY>` | チケット詳細表示 | `./scripts/jira.sh issue PROJ-123` |
| `create <project> <type> <summary> [desc]` | チケット作成 | `./scripts/jira.sh create PROJ Task "タスク名" "説明"` |
| `transitions <KEY>` | 遷移先一覧 | `./scripts/jira.sh transitions PROJ-123` |
| `transition <KEY> <id>` | ステータス変更 | `./scripts/jira.sh transition PROJ-123 31` |
| `comment <KEY> "<body>"` | コメント追加 | `./scripts/jira.sh comment PROJ-123 "確認しました"` |
| `projects` | プロジェクト一覧 | `./scripts/jira.sh projects` |
| `users "<query>"` | ユーザー検索 | `./scripts/jira.sh users "根本"` |

#### 秘書での使い方

秘書に話しかけるだけで Jira 操作ができる。

```
「自分のチケット見せて」      → オープンチケット一覧
「PROJ-123の詳細」           → チケット詳細
「○○のチケット作って」       → チケット作成（確認後）
「PROJ-123を完了にして」     → ステータス変更（確認後）
「期限切れチケットある？」    → 期限切れチケット検索
```

#### アーキテクチャ

Jira 連携は **専用スキル + シェルスクリプト** の2層構造。他のスキルは Jira スキルを呼び出すだけで、Jira の操作手順を知る必要がない。

```
┌──────────────────────────────────┐
│ 既存スキル                        │
│ /secretary, /morning, /evening   │
│ /generate-daily, /generate-weekly │
│ /extract-todo, /format-meeting   │
│          │                        │
│          │ スキル呼び出し          │
│          ▼                        │
│ Jira スキル                       │
...
Jira スキルは以下のスキルから呼び出される。

| 呼び出し元 | 使う Jira スキル | 用途 |
|-----------|-----------------|------|
| `/secretary` | `/jira-check`, `/jira-create-ticket`, `/jira-update-ticket`, `/jira-issue` | 対話型のチケット操作全般 |
| `/morning` | `/jira-check my-tickets`, `/jira-check overdue` | 朝のチケット確認・期限超過警告 |
| `/evening` | `/jira-check today`, `/jira-update-ticket` | 今日の更新まとめ・ステータス変更提案 |
| `/generate-daily` | `/jira-check daily` | 日次アジェンダに Jira セクション追加 |
| `/generate-weekly` | `/jira-check weekly` | 週次サマリーに Jira サマリー追加 |
| `/extract-todo` | `/jira-create-ticket` | TODO 抽出後に Jira 起票提案 |
| `/format-meeting` | `/jira-issue` | 議事録中のチケットキーをタイトル補完 |
| `/onboarding` | （直接 `jira.sh`） | Jira 接続テスト・初期設定 |

#### エラー時の動作

- API 接続エラー時は「Jira: 取得できませんでした」と表示してスキップ
- 認証エラー（401/403）→ API トークンの確認を案内
- レート制限（429）→ 自動リトライ（最大3回）
- チケット作成・変更などの破壊的操作は必ずユーザー確認後に実行
